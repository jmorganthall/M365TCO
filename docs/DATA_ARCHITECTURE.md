...
| **Secrets** (OpenRouter key) | Must be out of the relational store by nature | Behind the secret-store contract; write-only; swappable for Key Vault. Price-sheet sync stores no token. *Exception:* the Watchtower one-click-update token may instead be injected via operational env (`WATCHTOWER_TOKEN`) for a host-managed/shared updater — the encrypted store stays the preferred source and wins when both are set. |
